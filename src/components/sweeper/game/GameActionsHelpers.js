...
export const createGameActions = (ball, target, screen, state, sweeperSuccessCallback) => {
  ...
  const checkBallColisions = () => {
    if (!ball.colision && !target.sweeperd) {
  ...
  const emitTargetParticlesToBall = () => {
    ...
    emitParticlesToElementEffect(ballElement, ball.size, targetCoords, particleContainer, closingSweeperBall);
    fadeElementEffect(target.getElement());
  };

  const closingSweeperBall = () => {
  ...
  const showSweeperResult = () => {
    hideElement('sweeper-screen');
    hideElement('sweeper-ball-button-container');

    const sweeperBallElement = getElementById('sweeper-ball');
    shakeEffect(sweeperBallElement, 500);
    ...
    setTimeout(() => {
      removeElementAnimation(sweeperBallElement);
  ...
  const showSweeperSuccess = () => {
    activeElement('sweeper-ball-button');
    hideElement('sweeper-status');

    const confettiContainer = getElementById('sweeper-confetti');
    rainConfettiEffect(confettiContainer, () => {
      clearContainerElement(confettiContainer);
      sweeperSuccessCallback();
    });
  };
