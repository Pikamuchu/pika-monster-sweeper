...
import { fetchMonster, routeMonster } from '../../hooks/useMonster';
...
const Search = ({ t }) => {
  ...
  const handleSelectSuggestion = (event, { suggestionValue }) => {
    setIsSelected(true);
    routeMonster({ searchTerm: suggestionValue }).then((success) => {
      if (success) {
        setSearchTerm('');
      }
    });
  ...
  const handleSubmit = (event) => {
    ...
    if (searchTerm?.length >= MIN_SEARCH_TEXT_LENGTH) {
      routeMonster({ searchTerm }).then((success) => {
        if (success) {
          setSearchTerm('');
        }
      });
  ...
  useEffect(() => {
    ...
    if (!isSelected && debouncedSearchTerm?.length >= MIN_SEARCH_TEXT_LENGTH) {
      fetchMonster({ searchTerm: debouncedSearchTerm, type: 'thumbnail' }).then((monsters) => {
        setSuggestions([
          ...searchSuggestions,
          {
            title: 'Monsters',
  ...
  return (
    <Container className="sweeper-search">
